# Derived from https://github.com/rosinality/vq-vae-2-pytorch/blob/master/pixelsnail.py
...
class PixelSNAIL(nn.Module):
    def __init__(
        ...
        assert input_mode in ("vectors", "indices")
        ...
        self.horizontal = CausalConv2d(
            input_dim, channel, [kernel // 2, kernel], padding='down'
        )
        self.vertical = CausalConv2d(
            input_dim, channel, [(kernel + 1) // 2, kernel // 2], padding='downright'
        )
        ...
        if n_cond_res_block > 0:
            self.cond_resnet = CondResNet(
                input_dim, cond_res_channel, cond_res_kernel, n_cond_res_block
            )
    ...
    def forward(self, input, condition=None, cache=None, output_softmax: bool = False):
        if cache is None:
            cache = {}
